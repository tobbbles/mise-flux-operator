...
- `flux-operator` → your tool name (e.g., `semver`)
- `tobbbles` → your GitHub username or organization
- `mise-flux-operator` → the upstream tool's GitHub repository name
...
```bash
mise plugin link --force flux-operator .
```
...
Enable debug output:
```bash
MISE_DEBUG=1 mise install flux-operator@latest
```
